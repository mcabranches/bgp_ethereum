...
# endif
#else
#warning "Compiling Python2 extension module. Consider moving to Python3"
#endif
...
static PyObject *tcpResumeFunc = NULL;
...
register_resume_tcp() -- install a callback for resumed TCP sessions\n\
...
NIDS_TIMED_OUT, NIDS_TIMEOUT NIDS_RESUME -- possible values of the 'nids_state' member\n\
...
static void
callTcpResumeFunc(struct tcphdr *tcphdr, struct ip *iphdr, int *resume)
{
	PyObject *ret = NULL;

	DBG("callTcpResumeFunc...\n");
	*resume = NIDS_TCP_RESUME_NONE;

#if PY_MAJOR_VERSION >= 3
	ret = PyObject_CallFunction(tcpResumeFunc, "y#y#",
#else
	ret = PyObject_CallFunction(tcpResumeFunc, "s#s#",
#endif
		tcphdr, ntohs(iphdr->ip_len) - (iphdr->ip_hl << 2),
		iphdr, ntohs(iphdr->ip_len));

	if (ret) {
		switch (PyLong_AsUnsignedLongMask(ret)) {
...
callUdpFunc(struct tuple4 *addr, u_char *data, int len, struct ip *pkt)
{
	...
	DBG("callUdpFunc...\n");
#if PY_MAJOR_VERSION >= 3
	ret = PyObject_CallFunction(udpFunc, "(Ny#y#)",
#else
...
callIpFunc(struct ip *pkt)
{
	...
	DBG("callIpFunc...\n");
#if PY_MAJOR_VERSION >= 3
	ret = PyObject_CallFunction(ipFunc, "y#", pkt, ntohs(pkt->ip_len));
#else
...
#if PY_MAJOR_VERSION >= 3
	ret = PyObject_CallFunction(fragFunc, "y#", pkt, ntohs(pkt->ip_len));
#else
...
makeRegisterFunc(tcp_resume, tcpResumeFunc, callTcpResumeFunc);
...
#if PY_MAJOR_VERSION >= 3
	// sa = strdup(PyBytes_AS_STRING(addr));
	// if (_parse_prefix(sa, &ctl[i].netaddr, &ctl[i].mask) < 0) {
	// 	free(sa);
	// 	return -1;
	// }
	// free(sa);
	if (_parse_prefix(PyBytes_AS_STRING(PyUnicode_AsASCIIString(addr)), &ctl[i].netaddr, &ctl[i].mask) < 0)
		return -1;
#else
	if (_parse_prefix(PyString_AS_STRING(addr), &ctl[i].netaddr, &ctl[i].mask) < 0)
		return -1;
#endif
...
pynids_param(PyObject *na, PyObject *args)
{
	...
	if (int_p) {
		...
		if (v) *int_p = (int) PyLong_AsLong(v);
#else
...
#if PY_MAJOR_VERSION >=3
			*char_pp = (v == Py_None) ? NULL : strdup(PyBytes_AsString(PyUnicode_AsASCIIString(v)));
#else
...
#if PY_MAJOR_VERSION >= 3
static struct PyModuleDef pynidsdef = {
	...
	/* Add versioning info */
	PyModule_AddStringConstant(m, "__version__", "0.6.3");
	PyModule_AddObject(m, "__nids_version__",
#if PY_MAJOR_VERSION >= 3
						PyUnicode_FromFormat("%d.%d", NIDS_MAJOR, NIDS_MINOR));
#else
...
#define NIDS_TIMEOUT NIDS_TIMED_OUT
	...
	setConst(NIDS_RESUME);

	setConst(NIDS_TCP_RESUME_NONE);
	setConst(NIDS_TCP_RESUME_CLIENT);
	setConst(NIDS_TCP_RESUME_SERVER);
